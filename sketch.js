...
function draw() {
  ...
  if(keyDown(LEFT_ARROW)){
    ...
    balloon.addAnimation("si", balloonImage2);
  }
  else if(keyDown(RIGHT_ARROW)){
    updateHeight(10,0);
    // Agrega la animación del globo [usa balloonImage2]
    balloon.addAnimation("si1", balloonImage2);
  }
  else if(keyDown(UP_ARROW)){
    updateHeight(0,-10);
    // Agrega la animación del globo [usa balloonImage2]
    balloon.addAnimation("si2", balloonImage2);
    balloon.scale=balloon.scale -0.005;
  }
  else if(keyDown(DOWN_ARROW)){
    updateHeight(0,+10);
    // Agrega la animación del globo [usa balloonImage2]
    balloon.addAnimation("si3", balloonImage2);
    balloon.scale=balloon.scale+0.005;
  }
